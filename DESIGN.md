...
Uses standard AppKit controls, keyboard navigation, clear grouping, and immediate local persistence. Both scrolling surfaces use one scroll view: vertical only, with the horizontal lock in the clip view's `constrainBoundsRect` so a scroll is constrained before it happens rather than undone afterwards, no elasticity at either end, and opaque document views so the scroller copies what is drawn instead of repainting the visible page every frame. Nothing inside a scrolling page may scroll on its own: a nested scroller swallows the wheel while the pointer is over it, which reads as the page sticking. The window is the popover's size, 430 by 720, and scrolls: a settings page that grows a window past the bottom of a laptop screen is worse than one that scrolls, and a fixed size means no control can resize the window by appearing. It carries no banner of its own, because the title bar already names it. Reorder controls are arrow glyphs rather than spelled-out buttons, which is what fits beside a label column at that width; each keeps an accessibility label and a tooltip. `Clear` beside the shortcut recorder is hidden when no shortcut is set rather than shown disabled. Command-W closes the window and leaves the app running. While the window is open the app switches to the regular activation policy so it appears in the Command-Tab switcher and the Dock, and returns to accessory when it closes. Settings are grouped by what they change, under six headers in this order: `MENU BAR` (metrics, layout), `DASHBOARD` (card order), `SAMPLING` (interval, trend window), `SEAROOM CLI` (command, agent skills), `GENERAL` (shortcut, launch at login, trackpad feedback) and `MAINTENANCE` (check for updates, reset trend history). A header spans both columns; a row is a label and its control. Every toggle is a switch on the trailing edge, so the three of them line up rather than drifting with their labels, and the row label is the control's only name, which is why each switch carries an explicit accessibility label. The `SEAROOM CLI` switch is driven by the filesystem rather than a stored flag, so it reports what a terminal would find, and the agent-skills row is hidden entirely while the command is off. Include sample interval, trend window, the ordered menu-bar metric list, launch at login, global shortcut, and a confirmed action to reset trend history without changing preferences. The trend window is a slider rather than a menu because it now offers 26 stops, 15 and 30 minutes then every hour to 24, and a list that long is worse to scan than a track to drag. It snaps to tick values only, so every position is a real setting, and names the current value beside the track. On the first packaged launch only, a short native confirmation asks whether Searoom should open at login; both choices are explicit, remembered locally, and reversible here. Development-mode launches never show the confirmation. The menu-bar metric list adds, removes, and reorders up to five entries, offers only metrics not already chosen, accepts an empty selection as meaning the mark alone, and retains CPU / RAM / temperature as its initial default. There is no preview of the menu bar above it. One was tried and removed: the two layouts have different natural sizes, so switching between them resized the window, and a settings page that jumps when you change a setting costs more than the mockup was worth. The menu bar itself updates the moment the selection changes, which is the preview. Advanced sensor limitations belong in concise help text rather than visual warnings on every card. The footer keeps version and license status at the lower left, with quiet `GITHUB ↗` and `PART OF EMAITCHESS ↗` links at the lower right in that order. Both links use the micro-label style, remain keyboard and VoiceOver accessible, and open their destinations in the default browser only after explicit activation. The GitHub link targets the canonical repository. The affiliation URL retains stable `searoom` source, `desktop_app` medium, `product_attribution` campaign, and `settings_footer` content UTM values.
