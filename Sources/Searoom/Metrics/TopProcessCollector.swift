...
/// read, which covers processes that `proc_listallpids` hides; the readable
/// ones get one `proc_pidinfo` syscall each, and the rest are skipped.
final class TopProcessCollector {
    ...
    func read(interval: TimeInterval) -> ProcessRanking {
        ...
        guard let candidates = scan(hadBaseline: hadBaseline, elapsed: elapsed) else {
    ...
    /// was unreadable.
    private func scan(hadBaseline: Bool, elapsed: Double) -> [ProcessRanking.Candidate]? {
        ...
        for index in 0..<entryCount {
            ...
            // skip them rather than fabricating a zero.
