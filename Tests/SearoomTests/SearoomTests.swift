...
final class SearoomTests: XCTestCase {
    ...
    func testDefaultLayoutReproducesTheOriginalFixedGeometry() {
        ...
        let expected: [(DashboardSection, NSRect)] = [
            ...
            (.topProcesses, NSRect(x: 12, y: 586, width: 406, height: 122)),
    ...
    func testEverySectionOrderKeepsAllSectionsAndClearsTheHeader() {
        ...
        let orders: [[DashboardSection]] = [
            ...
            [.extras, .cpu, .info, .memory, .network, .gpu, .thermal, .gpuMemory, .disk, .topProcesses]
        ]
        for order in orders {
            let layout = DashboardLayout.make(order: order, width: 430)
    ...
    func testDropTargetFollowsThePointerAcrossRowsAndColumns() {
        ...
        XCTAssertEqual(
            ...
            [.memory, .gpu, .thermal, .gpuMemory, .disk, .cpu, .topProcesses, .network, .info, .extras]
        )
    ...
    func testDashboardSectionOrderRoundTripsAndMigrates() throws {
        ...
        settings.dashboardSectionOrder = [
            .gpuMemory, .cpu, .memory, .topProcesses, .gpu, .thermal, .disk, .network, .info, .extras
        ]
        let encoded = try JSONEncoder().encode(settings)
        XCTAssertEqual(try JSONDecoder().decode(AppSettings.self, from: encoded), settings)
    ...
    func testSectionOrderMigrationAdoptsTheNewDefaultForNeverReorderedArchives() throws {
        // An archive written while the card shipped after Memory.
        let afterMemory = Data(
            "{\"dashboardSectionOrder\":[\"cpu\",\"memory\",\"topProcesses\",\"gpu\",\"thermal\",\"gpuMemory\",\"disk\",\"network\",\"info\",\"extras\"]}".utf8
        )
        XCTAssertEqual(
            try JSONDecoder().decode(AppSettings.self, from: afterMemory).dashboardSectionOrder,
            DashboardSection.defaults
        )

        // A pre-card archive has the card appended by normalization; that
        // also counts as never reordered.
        let preCard = Data("{\"sampleInterval\":5}".utf8)
        XCTAssertEqual(
            try JSONDecoder().decode(AppSettings.self, from: preCard).dashboardSectionOrder,
            DashboardSection.defaults
        )

        // A deliberate reordering is never touched.
        let custom = Data(
            "{\"dashboardSectionOrder\":[\"topProcesses\",\"cpu\",\"memory\",\"gpu\",\"thermal\",\"gpuMemory\",\"disk\",\"network\",\"info\",\"extras\"]}".utf8
        )
        let reordered = try JSONDecoder().decode(AppSettings.self, from: custom)
        XCTAssertEqual(reordered.dashboardSectionOrder.first, .topProcesses)
        XCTAssertNotEqual(reordered.dashboardSectionOrder, DashboardSection.defaults)
    }
    ...
    func testProcessRankingOrdersCapsAndSkipsIdleConsumers() {
        let pool = [
        ...
        let ranked = ProcessRanking.rankedPIDs(in: pool)
        XCTAssertEqual(
            ranked.cpu,
            [11, 15, 13, 14, 10],
            "the CPU list is sorted strongest first, skips idle processes, and caps at five"
        )
        XCTAssertEqual(
            ranked.memory,
            [14, 11, 12, 13, 10],
            "the memory list is sorted largest first and caps at five, keeping idle processes"
        )

        let ties = [
            ProcessRanking.Candidate(pid: 20, cpuUsage: 0.2, residentBytes: 100),
            ProcessRanking.Candidate(pid: 21, cpuUsage: 0.2, residentBytes: 100),
            ProcessRanking.Candidate(pid: 22, cpuUsage: 0.2, residentBytes: 100)
        ]
        let tiedRanking = ProcessRanking.rankedPIDs(in: ties)
        XCTAssertEqual(
            tiedRanking.cpu,
            [20, 21, 22],
            "equal readings keep input order so ranking cannot jitter between reads"
        )
        XCTAssertEqual(
            tiedRanking.memory,
            [20, 21, 22]
        ...
        XCTAssertTrue(
            ProcessRanking.rankedPIDs(in: []).cpu.isEmpty
                && ProcessRanking.rankedPIDs(in: []).memory.isEmpty,
            "an empty scan ranks nothing"
        )
        XCTAssertEqual(ProcessRanking.maximumCount, 5, "the card shows five per column")
    }
