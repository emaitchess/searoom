...
- The Settings footer pairs the existing open-source/privacy line on the left with a quiet `PART OF EMAITCHESS ↗` link on the right. Open `https://emaitchess.com/` through `NSWorkspace` only after explicit activation; Searoom itself must not fetch the site. Preserve the outbound link's stable UTM attribution: source `searoom`, medium `desktop_app`, campaign `product_attribution`, and content `settings_footer`.
