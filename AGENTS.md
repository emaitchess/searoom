...
- Custom menu-bar mode contains one to three ordered, unique `MenuBarMetric` values. Its persisted default is CPU usage, RAM used, and temperature; normalize empty, duplicate, or oversized arrays before use. Changing the selected metrics may change the natural status-item width, but sampling updates reuse the same fixed value columns.
...
- The Settings footer pairs the version and open-source/privacy line on the left with a quiet `PART OF EMAITCHESS ↗` link on the right. The left line reads `SEAROOM <version> · OPEN SOURCE · MIT · NO ANALYTICS`, taking the version from the bundle so it cannot drift from the shipped build, and carries an accessibility label because the `·` separators read poorly aloud. Keep any privacy claim there true: it previously said `NO NETWORK ACCESS`, which the update check made false. Open `https://emaitchess.com/` through `NSWorkspace` only after explicit activation; Searoom itself must not fetch the site. Preserve the outbound link's stable UTM attribution: source `searoom`, medium `desktop_app`, campaign `product_attribution`, and content `settings_footer`.
