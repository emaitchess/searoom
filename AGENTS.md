...
| `Sources/Searoom/Models/DashboardSection.swift` | The ten movable dashboard sections, their geometry, and order normalization |
...
- Retained samples are capped at `AppModel.maximumStoredSamples`, the count a three hour window at a one second interval already produced. Windows longer than that keep their full wall-clock span and store every nth sample, so no window costs more than the longest one did before the trend slider reached 24 hours. Do not raise the cap to gain graph detail: the dashboard downsamples every series to its pixel width, under 400 points, before drawing. `currentSample` is never thinned; decimation applies only to what history retains.
...
- Live CLI sampling (`sample`, `watch`, `status`, `capabilities`) constructs one `SystemMetricsCollector`, discards one priming sample, and forces exactly one second disk-counter read. The priming pass also reads the process ranking once at the requested interval, so the emitted sample carries real per-process CPU rates across the same window. The ranking travels as the additive optional `sample.topProcesses` block, and only on live documents: persisted samples encode without the key. It never writes app history, settings, or launch-at-login state, and it never reaches the network.
- Options accept single-letter shorthands (`-h`, `-v`, `-j`, `-p`, `-i`, `-c`, `-n`, `-s`, `-u`, `-l`), defined in `CLIParser.shorthands` and resolved to the canonical long name before validation, so the per-command allowed sets, duplicate detection, and every error message are stated in the long form regardless of how the option was typed. `-h` or `--help` after a recognized command prints that command's reference and wins over validating the rest of the line; standalone `searoom -h` and `searoom -v` mirror `--help` and `--version`. The `help --json` catalog advertises each option shorthand in its `shorthand` field, encoded as explicit `null` when absent, and the bundled schema documents it as an optional additive field.
...
- Text menu-bar layouts use `NSStatusItem.variableLength`; do not restore a fixed width for the whole status item. Pad only volatile values to their compact character columns with `MetricFormat.fixedField`, and variable-width labels such as the temperature source with `MetricFormat.fixedLabel`, so that in monospaced Departure Mono ordinary sampling updates cannot move neighboring items. Labels retain natural width, values that exceed a field must expand rather than truncate, and adjacent metric groups use one subdued `·` with no surrounding spaces. Text layouts are leading-aligned and begin with the cached, fixed-size, non-template `SearoomStatusDot` colored from overall pressure. The dot is not the Searoom mark and must retain its stable leading width, light/dark variants, and accessible state description. Each metric component carries its own `MenuBarTone`: pressure colors for health signals, cool/subdued activity colors for network and disk I/O, and neutral ink only when magnitude has no health meaning. Do not turn high throughput or long uptime into a warning color. Keep the mark-only item, which is what an empty selection produces, centered and square; it is the only menu-bar mode that displays the Searoom mark.
...
- Centre a single line of text by cap height, not by its line box. These readings are uppercase and carry no descenders, so the line box holds slack below the baseline that nothing occupies, and centring the box leaves the text visibly high. `DashboardView.centredTextY(in:font:)` is the helper, `MenuBarRenderer` packs its stacked lines the same way, and `testTextIsCentredByCapHeightNotLineBox` guards it.
...
Release notes are assembled from `CHANGELOG.md`: `Scripts/release.sh` extracts the section for the version being released and refuses to publish if it is empty, so a release page always says what changed. Keep any claim in that template true. It previously said Searoom "has no updater and no network client", which the user-initiated update check had made false; the same claim was corrected in the Settings footer earlier and was missed here.
