...
The CLI (`Sources/Searoom/CLI`) runs outside this pipeline by design. `CLIRunner` constructs its own `SystemMetricsCollector` and calls it synchronously on the main thread — no `MetricsEngine`, no timer, no extra polling loop — which keeps the collector's stateful baselines single-threaded in the CLI process too. Do not route CLI sampling through `MetricsEngine`; its immediate first fire and main-actor callback exist for the app's update path.
...
- History: binary plist at `~/Library/Application Support/Searoom/history.plist`, encoded by the shared `HistoryArchiveStore`, written atomically on the `app.searoom.history` queue at most once per 60 seconds, wrapped in `Archive(version: 1)`. Persistence receives a deliberate contiguous snapshot of the ring rather than retaining its live storage. The CLI reads the same store synchronously under conservative file-size and sample-count limits; any other version or unreadable data fails closed to an empty history rather than blocking launch.
...
- Dashboard geometry comes from `DashboardLayout.make(order:width:)`, not from literals. It resolves the persisted `dashboardSectionOrder` into every rectangle plus the document height, and `draw`, both invalidation paths, and the unit regions read it. `testDefaultLayoutReproducesTheOriginalFixedGeometry` pins the default order to the coordinates that shipped before the layout was computed.
- Commit messages follow Conventional Commits (`feat:`, `fix:`, `ci:`, `docs:`, ...), subject in lowercase imperative with no trailing period, subject at most 72 characters, body wrapped at 80, trailers exempt. `.githooks/commit-msg` checks this when `git config core.hooksPath .githooks` has been set in the clone. History before the convention was adopted does not follow it.
