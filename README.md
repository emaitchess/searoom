...
- Top five processes by CPU and by resident memory, always shown on the dashboard, with selectable rows you can copy
...
- **Top Processes** ranks the five heaviest readable processes by CPU rate and by
  ...
  metric. Clicking a row selects it and Command-C copies the process name;
  Escape releases the selection. Enumeration is a public `sysctl kern.proc` read,
  wider than `proc_listallpids`; sandboxed helpers and other-user processes that
